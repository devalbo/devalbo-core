...
export const MarkdownViewEdit: React.FC<FileEditProps> = ({ content, onSave, onChange }) => {
  const incoming = useMemo(() => toMarkdownText(content), [content]);
  const [baseline, setBaseline] = useState(incoming);
  const [draft, setDraft] = useState(incoming);
  const [isSaving, setIsSaving] = useState(false);
  const dirty = draft !== baseline;
  const previewHtml = useMemo(() => renderMarkdownHtml(draft), [draft]);
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        ...
        <div style={{ background: '#020617', border: '1px solid #334155', borderRadius: '8px', padding: '12px', color: '#e2e8f0', lineHeight: 1.6 }}>
