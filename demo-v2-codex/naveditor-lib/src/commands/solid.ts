...
import type { AsyncCommandHandler, ExtendedCommandOptions } from './_util';
import { makeOutput, makeResult, makeResultError } from './_util';
...
> = {
  ...
  'solid-pod-push': async (_args, options) => {
    ...
    try {
      ...
      const result = await pushPodData(options.store, options.session, podNamespace);
      return makeResult(
  ...
  'solid-pod-pull': async (_args, options) => {
    ...
    try {
      const podNamespace = options.config?.podNamespace ?? 'devalbo';
      const result = await pullPodData(options.store, options.session, podNamespace);
      return makeResult(
