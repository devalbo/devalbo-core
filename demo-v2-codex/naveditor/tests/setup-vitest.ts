...
beforeAll(() => {
  ...
  vi.spyOn(console, 'error').mockImplementation((...args: Parameters<typeof console.error>) => {
    ...
    if (typeof firstArg === 'string') {
      ...
      if (firstArg.includes('inside a test was not wrapped in act(...)')) {
        return;
      }
      if (firstArg.includes('When testing, code that causes React state updates should be wrapped into act(...)')) {
        return;
      }
      if (firstArg.includes('This ensures that you\'re testing the behavior the user would see in the browser.')) {
        return;
      }
    }
