...
| `@devalbo/shared` | Core types (`AppConfig`, `CommandResult`, branded types) | Rarely — key types re-exported through cli-shell |
...
For the quickstart, you only need one import source: `@devalbo/cli-shell`.

---

## Quickstart: CLI-Only App (5 minutes)

This walkthrough builds a minimal CLI-only app. Code matches `examples/hello-cli/` exactly.
...
Install commands (pnpm):

```sh
pnpm add @devalbo/cli-shell commander react
pnpm add -D typescript tsx @types/node @types/react
```

If you want to consume `@devalbo/cli-shell` directly from the GitHub monorepo (instead of npm publish), use:

```sh
pnpm add @devalbo/cli-shell@github:devalbo/devalbo-core#main&path:packages/cli-shell
```

Notes:
- The `workspace:*` and `catalog:` versions shown in the JSON are for this monorepo setup.
- For standalone apps, use normal semver versions or the GitHub source form above.
...
Note: `pnpm start` requires a TTY terminal (interactive Ink shell). It will not work in CI or non-interactive shells.
...
All imported from `@devalbo/cli-shell`:

```ts
import { makeOutput, makeError, makeResult, makeResultError } from '@devalbo/cli-shell';
...
```ts
import type { StoreCommandHandler } from '@devalbo/cli-shell';
import { makeOutput, makeError } from '@devalbo/cli-shell';
...
```ts
import { createCliAppConfig } from '@devalbo/cli-shell';
...
### Browser/desktop apps with Solid: manual `AppConfig`

For apps that use Solid sync or need custom sync timings, build `AppConfig` manually:
...
export const defaultAppConfig: AppConfig = {
  ...
  sync: {
    social: { pollIntervalMs: unsafeAsMilliseconds(30_000), outboundDebounceMs: unsafeAsMilliseconds(1_000) },
    files: { pollIntervalMs: unsafeAsMilliseconds(30_000), outboundDebounceMs: unsafeAsMilliseconds(1_500), maxFileSizeBytes: unsafeAsByteCount(5 * 1024 * 1024) }
  },
  features: { socialSync: true, fileSync: true, fileSharing: false }
};
```

> **naveditor does this:** `editor-lib/src/web/config.ts` — manual `AppConfig` with custom sync timings and Solid features enabled.
...
import { defaultWelcomeMessage } from '@devalbo/cli-shell';
...
import { AppConfigProvider, createDevalboStore, useAppConfig, type DevalboStore } from '@devalbo/state';
import { createFilesystemDriver, type IFilesystemDriver } from '@devalbo/filesystem';
...
Key points:
- `InteractiveShell`, `bindCliRuntimeSource`, `unbindCliRuntimeSource` all import from `@devalbo/cli-shell`
...
Install: `@devalbo/solid-client`

Wrap your App in `SolidSessionProvider` and use `useSolidSession()` to get the active session:
...
// Pass session into InteractiveShell:
<InteractiveShell
  commands={commands}
  createProgram={createProgram}
  session={session}
...
// And into CLI runtime context:
getContext: () => ({
  store,
  session: sessionRef.current,  // now available to commands via options.session
  // ...
})
...
The `session` prop is typed as `unknown | null` in cli-shell to avoid a dependency on `@devalbo/solid-client`. Commands that need a typed session should cast: `options.session as SolidSession`.
...
| Command runtime | `src/lib/command-runtime.ts` | `@devalbo/cli-shell` |
| InteractiveShell component | `src/components/InteractiveShell.tsx` | `@devalbo/cli-shell` |
| Console helpers | `src/web/console-helpers.ts` | `@devalbo/cli-shell` |
